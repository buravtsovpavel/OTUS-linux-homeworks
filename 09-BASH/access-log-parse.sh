...
variables=/vagrant/variables.txt                 # файл для хранения значений времени последнего запуска и числа строк в логе при последнем запуске
accesslog=/vagrant/access-4560-644067.log        # основной лог
message_mail=/vagrant/message.txt                # файл который будем высылать на почту
lockfile=/vagrant/mylockfile  			              # локфайл
...
if ( set -o noclobber; echo "$$" > "$lockfile") 2> /dev/null; then   
  ...
  if [ -e $accesslog ]; then                                          
    ...
    line_counter=$(cat /vagrant/variables.txt | grep line_counter | awk -F"=" '{print $2}')  
    last_running_date=$(cat /vagrant/variables.txt | grep last_running_date | awk -F"=" '{print $2}')  
